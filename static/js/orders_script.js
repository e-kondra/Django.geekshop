window.onload = function () {
    ...
    let total_forms = parseInt($('input[name=orderitems-TOTAL_FORMS]').val()); // общее кол-во строк
    console.log(total_forms);

    let order_total_quantity = parseInt($('.order_total_quantity').text()) || 0;// общее количество предметов в заказе
    let order_total_price = parseInt($('.order_total_cost').text().replace(',', '.')) || 0; // общая сумма
    ...
    $('.order_form').on('click', 'input[type=number]', function () {
        ...
        if (price_arr[orderitem_num]) {
            ...
            OrderSumUpdate(price_arr[orderitem_num], delta_quantity)
        }
    ...
    function OrderSumUpdate(orderitem_price, delta_quantity) {
    ...
    //$('.order_form select').changed(function(){
